...
from aero_pi_ogn_receiver.setup.setup import INSTALL_STATE_FILENAME, SYSTEMD_UNITS, SetupPaths
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m aero_pi_ogn_receiver.setup.uninstall",
        description="Remove aero-pi-ogn-receiver system integration.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "state, and logs while preserving /etc/aero-pi-ogn-receiver/config.yaml"
        ),
    )
    parser.add_argument(
        "--remove-binaries",
        action="store_true",
        help="Remove /opt/aero-pi-ogn-receiver",
    )
    parser.add_argument(
        "--remove-packages",
        action="store_true",
...
def run_uninstall(options: UninstallOptions) -> None:
    if options.dry_run:
        ...
        print("Uninstalling aero-pi-ogn-receiver system integration:")
...
def _stop_and_disable(options: UninstallOptions) -> None:
    ...
    if not shutil.which("systemctl"):
        ...
        return
    for command in (
        ["systemctl", "stop", "aero-pi-ogn-receiver.target"],
        ["systemctl", "disable", "aero-pi-ogn-receiver.target"],
    ):
