...
from aero_pi_ogn_receiver.core import paths as default_paths
from aero_pi_ogn_receiver.core.architecture import (
    SUPPORTED_BINARY_ARCHES,
    host_os_architecture,
    resolve_binary_arch,
)
from aero_pi_ogn_receiver.core.checksums import ChecksumMismatch, verify_file_hash
from aero_pi_ogn_receiver.core.config_model import load_config
from aero_pi_ogn_receiver.core.manifest import BinaryEntry, load_manifest
from aero_pi_ogn_receiver.core.render import render_ogn_config
from aero_pi_ogn_receiver.setup import venv_readme
...
SYSTEMD_UNITS = (
    "aero-pi-ogn-rf.service",
    "aero-pi-ogn-decode.service",
    "aero-pi-ogn-receiver.target",
)
...
class SetupPaths:
    ...
    def under_root(cls, root: Path) -> "SetupPaths":
        return cls(
            config_dir=root / "etc/aero-pi-ogn-receiver",
            config_path=root / "etc/aero-pi-ogn-receiver/config.yaml",
            native_config_path=root / "etc/aero-pi-ogn-receiver/rtlsdr-ogn.conf",
            opt_dir=root / "opt/aero-pi-ogn-receiver",
            ogn_install_dir=root / "opt/aero-pi-ogn-receiver/ogn",
            ogn_current_dir=root / "opt/aero-pi-ogn-receiver/ogn/current",
            state_dir=root / "var/lib/aero-pi-ogn-receiver",
            cache_dir=root / "var/lib/aero-pi-ogn-receiver/cache",
            log_dir=root / "var/log/aero-pi-ogn-receiver",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m aero_pi_ogn_receiver.setup.setup",
        description="Prepare a Raspberry Pi for aero-pi-ogn-receiver.",
    )
...
def run_setup(options: SetupOptions) -> None:
    if options.dry_run:
        ...
        print("Setting up aero-pi-ogn-receiver:")
...
def _print_next_steps(options: SetupOptions) -> None:
    if options.dry_run:
        return
    aero_pi_ogn = Path(sys.executable).with_name("aero-pi-ogn")
    aero_pi_ogn_command = str(aero_pi_ogn) if aero_pi_ogn.exists() else "aero-pi-ogn"
    print()
    print("Setup complete. Next steps:")
    print(f"1. Edit configuration: sudo nano {options.paths.config_path}")
    print(
        "2. Validate config: "
        f"{aero_pi_ogn_command} config validate --config {options.paths.config_path}"
    )
    print(
        "3. Review rendered native config: "
        f"{aero_pi_ogn_command} config render --config {options.paths.config_path}"
    )
    print("4. Start services when ready: sudo systemctl enable --now aero-pi-ogn-receiver.target")
    print(f"5. Check status: {aero_pi_ogn_command} status --live")
