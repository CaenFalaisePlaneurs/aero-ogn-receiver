...
```text
bash: /home/pi/aero-ogn-receiver-venv/bin/activate: No such file or directory
```
...
```text
rf service          WARN     aero-ogn-rf.service inactive/dead
decode service      WARN     aero-ogn-decode.service inactive/dead
...
HTTP status page is not reachable. Check `aero-ogn status --live`.
...
error. Check receiver health with `aero-ogn status --live`.
...
open another terminal and run `aero-ogn status --live`.
